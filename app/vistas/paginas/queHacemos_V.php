<!-- Se carga el header -->
<?php include(RUTA_APP . "/vistas/inc/header_V.php");  ?>

		<div class="container contenedor_1">
            <div class="contenedor_15">
                <h1 class="h1_2">¿Porque hacemos y trabajamos en Ciudad Transparente?</h1>
                <p class="p_1">El tiempo nos ha demostrado con mucho dolor y padecimiento, que los politicos venezolanos acostumbran a negociar las instituciones publicas, administran los recursos economicos de la nación en la oscuridad, permiten la degradación de los recursos naturales, violan las leyes y ocultan los hechos, y disfrazan las malas practicas con la compra de voluntades, el populismo, las regalias, el marketing y la psicologia de la manipulación.</p>

                <p class="p_1">Partiendo de estas caracteristicas generales, estos personajes en realidad son el inicio y la continuidad de nuestro subdesarrollo, teniendo en cuenta que el desarrollo es obra de grupos de personas organizadas que colocan voluntad, inteligencia, motivación, tenacidad y creatividad en las actividades que emprenden, deberíamos dejar de escuchar promesas politicas y procurar trabajar juntos en nuestras soluciones.</p>

                <p class="p_1">En "Ciudad Transparente" queremos cambiar este ciclo nocivo en el que actualmente vivimos y encaminarmos hacia un nuevo orden social con las intenciones reales de lograrlo, esta plataforma es el inicio de una serie de contrapropuestas radicales, modernas e innovadoras que el movimiento se trae entre manos.</p>

                <p class="p_1">Nuestro próximo pequeño paso depende mucho de tí, conoce los detalles. <a class="boton_1"  href="<?php echo RUTA_URL . '/NuestroADN_C/pablins_ky';?>">Ver</a></p>
            </div> 
        </div>

<!-- Se carga el footer -->
<?php require(RUTA_APP . "/vistas/inc/footer_V.php"); ?>

<script type="text/javascript" src="<?php echo RUTA_URL . '/public/javascript/scrollUp.js';?>"></script>